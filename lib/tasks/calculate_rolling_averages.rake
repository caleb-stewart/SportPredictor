...
namespace :whl_rolling_averages do
  ...
  task calculate: :environment do
    ...
    WhlTeam.find_each do |team|
      ...
      games = WhlGame.where("home_team_id = ? OR away_team_id = ?", team.id, team.id).order(:game_date_iso_8601)
      games.each_with_index do |game, idx|
        ks.each do |k|
          ...
          WhlRollingAverage.create!(
            ...
            penalty_kill_percentage_avg: nil, # Set to nil unless you have this stat available
            goals_diff: goals_diff,
            ppp_diff: ppp_diff,
            sog_diff: sog_diff,
            fowp_diff: fowp_diff,
            target_fowp: avg.call(target_stats[:fowp]),
            opponent_fowp: avg.call(opponent_stats[:fowp]),
            home_away: game.home_team_id == team.id ? 1 : 0
          )
        end
      end
    end
